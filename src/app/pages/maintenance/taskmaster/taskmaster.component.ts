...
export class taskmasterComponent implements OnInit {
  ...
  table_config: any = {
    ...
    columns: [
      ...
      { name: 'create_at', displayName: '建立日期', templateRef: 'date' },
      { name: 'close_date', displayName: '預計完成', templateRef: 'date' },
  ...
  statusConfig = {
   ...
   status: {
  '待辦':   { color: '#ffffff', bg: '#d87a06', icon: 'schedule',     type: 'status-dot' },
  '進行中': { color: '#ffffff', bg: '#007bff', icon: 'play_arrow',   type: 'status-dot' },
  '測試':   { color: '#ffffff', bg: '#6f42c1', icon: 'biotech',      type: 'status-dot' },
  '審核':   { color: '#ffffff', bg: '#17a2b8', icon: 'fact_check',   type: 'status-dot' },
  '完成':   { color: '#ffffff', bg: '#28a745', icon: 'check_circle', type: 'status-dot' }
}
  };
...
onAdd() {
  const modalRef = this.modalSvc.open(taskmasterModalComponent, { 
    windowClass: "modal-mySize", 
    backdrop: 'static' 
  });
  modalRef.componentInstance.title = "新增";

  modalRef.result.then((success: any) => {
    // 只要子元件傳回 true，代表資料庫存檔與圖片上傳都搞定了
    if (success) {
      this.showSuccessToast('新增成功');
      this.loadData(); // 重新整理列表
    }
  }).catch(() => { });
}
...
onEdit() {
  ...
  calculateStats(data: any[]) {
    ...
    this.stats = {
      todo: data.filter(x => x.status === '待辦').length,
      doing: data.filter(x => x.status === '審核').length,
      done: data.filter(x => x.status === '完成').length,
      urgent: data.filter(x => x.priority === '緊急').length
    };
  }
