...
@Component({
  templateUrl: './psmeternoinfo-modal.component.html',
})
export class PsmeternoinfoModalComponent implements OnInit {
  @Input() formData: any={};
  @Input() categorys!: any[];
  @Input() psmeternoinfo: any[] = [];
  ...
  isExists(item: string){
    return this.psmeternoinfo.find(a => a.customer_name.substring(0, 2) == this.formGroup.get(item).value.substring(0, 2))
  }
  submit(){
  ...
  uniqueId(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} | null =>
      this.psmeternoinfo.map(t => t.cust_id).includes(control.value) ? {uniqued: control.value} : null;
  }
