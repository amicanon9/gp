...
@Component({
  templateUrl: './servicenoinfo-modal.component.html',
})


export class ServicenoinfoModalComponent implements OnInit {
  @Input() formData: any={};
  @Input() categorys!: any[];
  @Input() servicenoinfo: any[] = [];
  @Input() title: String = "{ERRRO}";
  @Input() booklist:any;
  formGroup = this.fb.group({
    id: [-1],
    book_id: [null, Validators.required],
    service_no: [null, [Validators.required, Validators.maxLength(20)]],
    ps_total_kwp: [null, [Validators.required, decimalValidator(3)]],
    pp_rate: [null, [decimalValidator(4)]],
  ...
  isExists(item: string){
    return this.servicenoinfo.find(a => a.customer_name.substring(0, 2) == this.formGroup.get(item).value.substring(0, 2))
  }
  submit(){
  ...
  uniqueId(): ValidatorFn {
    return (control: AbstractControl): {[key: string]: any} | null =>
      this.servicenoinfo.map(t => t.cust_id).includes(control.value) ? {uniqued: control.value} : null;
  }
