...
import { BankinfoModalComponent } from './bankinfo-modal/bankinfo-modal.component';
import { ActivatedRoute, Router } from '@angular/router';
@Component({
  selector: 'sepvdb-bankinfo',
...
export class BankinfoComponent implements OnInit {
  ...
  ngOnInit() {
    this.signalRSvc.StartConnection()
        this.signalRSvc.ReceiveListener()?.on('Bankinfo', (data) => {
          this.onDataRefresh();
        })
    this.loadData();
  }
  ...
  onAdd() {
    const modalRef = this.modalSvc.open(BankinfoModalComponent, { windowClass: "modal-mySize", backdrop: 'static' });
    modalRef.componentInstance.title = "新增";
    
    modalRef.result.then((res: any) => {
      let data = res;
   
      this.apiSvc.createbankinfo(data).pipe(
        catchError(err => {
          var errorMessage = '無法連接伺服器，請聯絡管理人員';
          if(err.status == 400){
            errorMessage = '新增失敗，重複的銀行代號';
          }
          this.toastr.error(
  ...
  onEdit() {
    ...
    modalRef.result.then((res: any) => {
      let data = res;
      this.apiSvc.updatebankinfo(this.selected.id, data).pipe(
        catchError(err => {
         var errorMessage = '無法連接伺服器，請聯絡管理人員';
          if(err.status == 400){
            errorMessage = '編輯失敗，重複的銀行代號';
          }
          this.toastr.error(
            '<span data-notify="icon" class="nc-icon nc-bell-55"></span><span data-notify="message">' +
            errorMessage
  ...
  horizontalPosition: 'center',    // 可選：start、center、end、left、right
      ...
      this.apiSvc.deletebankinfo(this.selected.id).pipe(
        catchError(err => {
          var errorMessage = '無法連接伺服器，請聯絡管理人員';
          if(err.status == 400){
            errorMessage = '刪除失敗，需先刪除對應銀行分行資料';
          }
          this.toastr.error(
            '<span data-notify="icon" class="nc-icon nc-bell-55"></span><span data-notify="message">' +
            errorMessage
