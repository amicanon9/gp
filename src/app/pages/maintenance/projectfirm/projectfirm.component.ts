...
// 假設你之後會建立這個 Modal
import { projectfirmModalComponent } from './projectfirm-modal/projectfirm-modal.component'; 

@Component({
  selector: 'sepvdb-projectfirm',
  templateUrl: './projectfirm.component.html',
  styleUrls: ['./projectfirm.component.scss']
})
export class projectfirmComponent implements OnInit, OnDestroy {
  search: string;
  projectfirmData: any[] = [];
 ...
 dataSource!: MatTableDataSource<any>;
  ...
  async ngOnInit() {
    try {
      ...
      // 監聽內部專案的 SignalR 事件
      this.signalRSvc.Hub.on('projectfirm', this.refreshData);
      this.loadData();
    } catch (err) {
  ...
  ngOnDestroy() {
    // 移除監聽，防止記憶體洩漏
    this.signalRSvc.Hub.off('projectfirm', this.refreshData);
  }
  ...
  onAdd() {
    const modalRef = this.modalSvc.open(projectfirmModalComponent, { windowClass: "modal-mySize", backdrop: 'static' });
    modalRef.componentInstance.title = "新增內部專案";
    modalRef.componentInstance.booklist = JSON.parse(JSON.stringify(this.booklist));
    modalRef.result.then((res: any) => {
      this.apiSvc.createdata('projectfirm', res).pipe(
        catchError(err => {
  ...
  onEdit() {
    ...
    modalRef.componentInstance.booklist = JSON.parse(JSON.stringify(this.booklist));
    modalRef.result.then((res: any) => {
      this.apiSvc.updatedata('projectfirm', this.selected.id, res).pipe(
        catchError(err => {
  ...
  onDelete() {
    ...
    ref.onAction().subscribe(() => {
      this.apiSvc.deletedata('projectfirm', this.selected.id).pipe(
        catchError(err => {
...
loadData() {
  ...
  forkJoin({
    projectData: this.apiSvc.getdata('projectfirm'),
    ...
    this.projectfirmData = projectData;
